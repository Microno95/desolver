...
with de.BlockTimer(section_label="Integrator Tests"):
    for i in sorted(set(de.available_methods.values()), key=lambda x:x.__name__):
        try:
            yi1 = torch.tensor([0.0, 1.0], requires_grad=True)
            df  = SimpleODE(k=1.0)

            a = de.OdeSystem(df, yi1, t=(0, 10), dt=0.01, rtol=1e-8, atol=1e-8)
            a.set_method(i)
            a.integrate(eta=True)

            dyfdyi = D.jacobian(a[-1].y, a[0].y)
            dyi = D.array([0.0, 1.0]) * 1e-5
            dyf = D.einsum("nk,k->n", dyfdyi, dyi)
            yi2 = yi1 + dyi

            b = de.OdeSystem(df, yi2, t=(0, 10), dt=0.01, rtol=1e-8, atol=1e-8)
            b.set_method(i)
