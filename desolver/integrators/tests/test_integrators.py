...
#         assert integrator_instance.adaptive == (integrator_instance.tableau_final.shape[0] == 2)
#     else:
#         assert integrator_instance.adaptive == False
#     if issubclass(integrator, RungeKuttaIntegrator):
#         assert integrator_instance.explicit == all([D.ar_numpy.all(integrator_instance.tableau_intermediate[col, col+1:] == 0.0) for col in range(integrator_instance.tableau_intermediate.shape[0])])
#         assert integrator_instance.fsal == D.ar_numpy.all(integrator_instance.tableau_intermediate[-1, 1:] == integrator_instance.tableau_final[0, 1:])
