...
def test_float_formats():
    for ffmt in D.available_float_fmt():
        ...
        y_init = D.array([pyaudi.gdual_double(1., 'x', 7), pyaudi.gdual_double(0., 'vx', 7)])
        ...
        with de.utilities.BlockTimer(section_label="Integrator Tests") as sttimer:
            for i in sorted(set(de.available_methods.values()), key=lambda x:x.__name__):
                try:
                    ...
                    max_diff = D.max(D.to_float(D.abs(analytic_soln(a.t[-1], a.y[0])-a.y[-1])))
...
def test_getter_setters():
    for ffmt in D.available_float_fmt():
        ...
        assert(D.to_float(D.norm(a.y[0]  - y_init)) <= 2 * D.epsilon())
        assert(D.to_float(D.norm(a.y[-1] - y_init)) <= 2 * D.epsilon())
        ...
        with de.utilities.BlockTimer(section_label="Integrator Tests") as sttimer:
            for i in sorted(set(de.available_methods.values()), key=lambda x:x.__name__):
                try:
                    ...
                    if D.to_float(D.abs(a.t[-1] - D.pi/8)) > 10*D.epsilon():
